...
- Body — `partially/compositely graded`: raw size remains descriptive; v1 derives graded waist/shoulders, waist/hips and waist/height metrics plus context-only chest/waist.
- Appearance — `contextual-only`: PARS and appearance anchors do not silently inherit RAPS grading.
...
- Sexual Anatomy & Physiology — `contextual-only / not gradeable` by default; anatomy, drive, arousal and rolling activity counts do not receive quality grades merely because values are numeric.
...
- `body.chest_to_waist_ratio`: derived context only in v1 because the literature supports shape relevance but this slice does not encode an unsupported single universal optimum.
...
Runtime candidate `5d893ff58429d9ccc53c57c04c73788319791250` passed:
- CI #765 / `31865573821`;
- Read-Only Grading Proof #28 / `31865573852`;
- Attribute Grading Batch 1 Acceptance #27 / `31865573805` on a disposable production copy;
- Public Readiness Security Audit #52 / `31865573804`.

Acceptance verifies raw profile and skill persistence remain unchanged while query/Telegram grading is derived at read time.
