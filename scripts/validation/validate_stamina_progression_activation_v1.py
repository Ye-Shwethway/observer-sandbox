...
from observer_sandbox.stamina_progression import STAMINA_FIELD_KEY
...
def main() -> int:
    ...
    set_field(conn, ACTOR, "physiology.fatigue", 0.0, authority="physiology_engine", source="stamina-activation-acceptance")
    conn.commit()

    state0 = snapshot(conn, ACTOR)
    bootstrap = maybe_settle_stamina_progression(conn, ACTOR, as_of_sim_time=str(state0["sim_time"]), state=state0)
    ...
    early = stamina_progression_due(conn, ACTOR, as_of_sim_time=early_time, state=snapshot(conn, ACTOR))
    ...
    due = stamina_progression_due(conn, ACTOR, as_of_sim_time=eligible_time, state=eligible_state)
    assert due.reason == "eligible_stimulus"
    assert due.eligible_stimulus_event_ids == (event_id,)

    activated = maybe_settle_stamina_progression(conn, ACTOR, as_of_sim_time=eligible_time, state=eligible_state)
    ...
    replay = maybe_settle_stamina_progression(conn, ACTOR, as_of_sim_time=eligible_time, state=snapshot(conn, ACTOR))
    ...
    print(json.dumps({
        ...
        "bootstrap_no_retroactive_credit": True,
