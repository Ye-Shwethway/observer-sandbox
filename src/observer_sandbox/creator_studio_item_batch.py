...
def ai_item_batch_draft(conn: sqlite3.Connection, user_id: int, prompt_text: str, *, sandbox_id: str = DEFAULT_SANDBOX_ID) -> dict[str, Any]:
    ...
    def validate_candidate(candidate: dict[str, Any]) -> None:
        if set(candidate) != {"items"}:
            raise ValueError("Creation AI returned an invalid Item batch envelope")
    ...
    try:
        candidate = generate_validated_item_candidate(
            ...
            canonicalize=canonicalize_ai_item_batch_fill,
...
def approve_item_batch_draft(conn: sqlite3.Connection, user_id: int, expected_revision: int, *, sandbox_id: str = DEFAULT_SANDBOX_ID) -> list[dict[str, Any]]:
    ...
    try:
        created = create_sandbox_item_batch(conn, entries, sandbox_id=sandbox_id, provenance_mode=str(draft.get("draft_mode") or "manual"), requested_by=f"telegram:{int(user_id)}")
    except (ValueError, TypeError, KeyError) as exc:
