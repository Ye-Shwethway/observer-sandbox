...
PASSIVE_DRIFT_PER_HOUR = {"energy": -2.0, "hunger": 2.5, "thirst": 3.0, "sleepiness": 3.0, "cleanliness": -0.8}
ACTION_EFFECTS_PER_HOUR: dict[str, dict[str, float]] = {
    ...
    "read": {"energy": -0.5}, "idle": {"energy": 3.0},
}
...
def action_definition(conn: sqlite3.Connection, action_type: str) -> dict[str, Any]:
    ...
    return {
        "action_type": row["action_type"], "label": row["label"],
        "min_duration_minutes": int(row["min_duration_minutes"]),
        "max_duration_minutes": int(row["max_duration_minutes"]),
        "target_mode": row["target_mode"], "required_capability": row["required_capability"],
...
def local_objects(conn: sqlite3.Connection, room_id: str) -> list[dict[str, Any]]:
    rows = conn.execute(
        """SELECT e.id,e.name,e.capabilities_json,e.definition_id FROM relations r JOIN entities e ON e.id=r.target_id
        WHERE r.source_id=? AND r.relation_type='contains' AND e.entity_type='object' ORDER BY e.id""", (room_id,),
    ).fetchall()
    return [{"id": r["id"], "name": r["name"], "definition_id": r["definition_id"],
             "capabilities": json.loads(r["capabilities_json"]), "effects": _object_effects(conn, r["id"])} for r in rows]
...
def reachable_rooms(conn: sqlite3.Connection, room_id: str) -> list[dict[str, str]]:
    rows = conn.execute(
        ...
        WHERE r.source_id=? AND r.relation_type='connected_to' AND e.entity_type='location' ORDER BY e.id""", (room_id,),
    ).fetchall()
...
def action_options(conn: sqlite3.Connection, actor_id: str = "char_darian") -> list[dict[str, Any]]:
    ...
    definitions = {row["action_type"]: action_definition(conn, row["action_type"]) for row in conn.execute("SELECT action_type FROM action_definitions")}
    for obj in local_objects(conn, room_id):
        for name, definition in definitions.items():
            ...
            if capability and capability in obj["capabilities"]:
                option = {"action": name, "target": obj["id"], "target_name": obj["name"],
    ...
    for name in ("rest", "idle"):
        ...
        option = {"action": name, "target": None, "target_name": None,
                  "duration": (definition["min_duration_minutes"], definition["max_duration_minutes"])}
...
def validate_action(conn: sqlite3.Connection, actor_id: str, action: Action) -> None:
    ...
    row = conn.execute(
        ...
        WHERE r.source_id=? AND r.relation_type='contains' AND e.id=? AND e.entity_type='object'""", (location, action.target),
    ).fetchone()
    if row is None:
...
def _advance_needs(conn: sqlite3.Connection, actor_id: str, action: Action) -> None:
    ...
    values = {"energy": float(get_field(conn, actor_id, "needs.energy", 75.0)), "hunger": float(get_field(conn, actor_id, "needs.hunger", 20.0)),
              "thirst": float(get_field(conn, actor_id, "needs.thirst", 15.0)), "sleepiness": float(get_field(conn, actor_id, "needs.sleepiness", 15.0)),
              "cleanliness": float(get_field(conn, actor_id, "physiology.cleanliness", 80.0))}
    for stat, rate in PASSIVE_DRIFT_PER_HOUR.items(): values[stat] += rate * hours
    for stat, rate in ACTION_EFFECTS_PER_HOUR.get(action.name, {}).items(): values[stat] += rate * hours
    if action.target:
        effects = _object_effects(conn, action.target).get(action.name, {})
        if isinstance(effects, dict): _apply_effect_spec(values, effects)
    set_field(conn, actor_id, "needs.energy", _clamp(values["energy"])); set_field(conn, actor_id, "needs.hunger", _clamp(values["hunger"]))
    set_field(conn, actor_id, "needs.thirst", _clamp(values["thirst"])); set_field(conn, actor_id, "needs.sleepiness", _clamp(values["sleepiness"]))
...
def _state_changes(before: dict[str, Any], after: dict[str, Any]) -> dict[str, Any]:
    result: dict[str, Any] = {}
    for key in ("location", "energy", "hunger", "thirst", "sleepiness", "cleanliness"):
        if before.get(key) != after.get(key): result[key] = {"before": before.get(key), "after": after.get(key)}
    return result


def ensure_action_instance(conn: sqlite3.Connection, action: Action, actor_id: str, *, action_id: str | None = None,
                           status: str = "in_progress", planned_wall_time: float | None = None,
                           due_wall_time: float | None = None, speed_at_plan: float | None = None) -> str:
    ...
    conn.execute(
        ...
        (action_id, action.name, actor_id, before["location"], action.target, status, action.duration_minutes, action.reason,
         json.dumps(list(action.participants)), json.dumps(list(action.resources)), json.dumps(action.conditions), json.dumps(action.modifiers),
         before["sim_time"], before["sim_time"] if status == "in_progress" else None, planned_wall_time, due_wall_time, speed_at_plan),
    )
    for participant in action.participants:
        conn.execute("INSERT OR IGNORE INTO action_participants(action_id,entity_id,role) VALUES(?,?,?)", (action_id, participant, "participant"))
    return action_id


def apply_action(conn: sqlite3.Connection, action: Action, actor_id: str = "char_darian", *, action_id: str | None = None) -> dict[str, Any]:
    if action_id and conn.execute("SELECT 1 FROM action_instances WHERE id=? AND status='completed'", (action_id,)).fetchone():
    ...
    participants = [{"entity_id": p, "role": "participant"} for p in action.participants]
...
def _move_toward(conn: sqlite3.Connection, location: str, destination: str, reason: str) -> Action:
    ...
    queue: deque[tuple[str, str | None]] = deque([(location, None)]); seen = {location}
    while queue:
        ...
        for neighbor in reachable_rooms(conn, node):
            ...
            if target == destination: return Action("move", 5, hop, reason)
            seen.add(target); queue.append((target, hop))
...
class BaselineLivingPolicy:
    ...
    def choose(self, state: dict[str, Any], available_actions: list[str]) -> Action:
        ...
        if state["sleepiness"] >= 72: return Action("sleep", 480, MASTER_BED, "recover from high sleep pressure") if location == MASTER_SUITE else _move_toward(self.conn, location, MASTER_SUITE, "go to bed")
        if state["energy"] <= 28: return Action("rest", 60, None, "recover low energy")
        if state["thirst"] >= 55: return Action("drink", 5, DRINKING_WATER, "respond to thirst") if location == KITCHEN else _move_toward(self.conn, location, KITCHEN, "get water")
        if state["hunger"] >= 60: return Action("eat", 25, MEAL_INGREDIENTS, "respond to hunger") if location == KITCHEN else _move_toward(self.conn, location, KITCHEN, "get food")
        if state["cleanliness"] <= 45: return Action("shower", 15, MASTER_SHOWER, "restore cleanliness") if location == MASTER_BATHROOM else _move_toward(self.conn, location, MASTER_BATHROOM, "take a shower")
        hour = datetime.fromisoformat(state["sim_time"]).hour
        if 8 <= hour < 11 and state["energy"] >= 50: return Action("train", 60, FREE_WEIGHTS, "morning training block") if location == HOME_GYM else _move_toward(self.conn, location, HOME_GYM, "start morning training")
        if 19 <= hour < 22: return Action("read", 45, LIBRARY_BOOKSHELF, "evening wind-down") if location == LIBRARY else _move_toward(self.conn, location, LIBRARY, "wind down with reading")
        if location == LIVING_ROOM: return Action("rest", 30, LIVING_SOFA, "unstructured recovery time")
...
def run_until(conn: sqlite3.Connection, end_time: datetime, *, decision_provider: DecisionProvider | None = None, actor_id: str = "char_darian", max_actions: int = 200) -> list[dict[str, Any]]:
    provider = decision_provider or BaselineLivingPolicy(conn); trace: list[dict[str, Any]] = []
    for _ in range(max_actions):
        state = snapshot(conn, actor_id); now = datetime.fromisoformat(state["sim_time"])
        if now >= end_time: break
        action = provider.choose(state, ACTION_NAMES); remaining = max(1, int((end_time - now).total_seconds() // 60))
        if action.duration_minutes > remaining:
            definition = action_definition(conn, action.name); low = definition["min_duration_minutes"]
            idle_def = action_definition(conn, "idle")
            action = Action("idle", min(remaining, idle_def["max_duration_minutes"]), None, "finish bounded simulation window") if remaining < low else Action(action.name, remaining, action.target, action.reason, action.participants, action.resources, action.conditions, action.modifiers)
...
def run_one_simulated_day(conn: sqlite3.Connection, actor_id: str = "char_darian") -> list[dict[str, Any]]:
    start = ensure_sim_clock(conn); return run_until(conn, start + timedelta(hours=24), actor_id=actor_id)
