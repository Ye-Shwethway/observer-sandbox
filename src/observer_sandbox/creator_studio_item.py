...
def manual_item_draft(
    ...
    *,
    ...
    return _save_draft(
        conn,
        user_id,
        _wrap_item_proposal(candidate, mode="manual", user_id=user_id),
        draft_mode="manual",
        prompt_text=None,
    ...
    *,
    ...
    try:
        ...
        raise CreatorStudioError(
            f"Creation AI Item failed exact validation after self-correction: {exc}"
        ) from exc
    return _save_draft(
        conn,
        user_id,
        _wrap_item_proposal(candidate, mode="ai_generated", user_id=user_id),
