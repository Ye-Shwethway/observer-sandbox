...
TASK_ID = "tactical_situation_assessment_sim_v1"
SIMULATOR_ID = "obj_thorne_estate_intel_tactical_situation_assessment_simulator"
SIMULATOR_DEFINITION_ID = "represented_task:tactical_situation_assessment_simulator_v1"
...
SOURCE = "tactical-assessment-task-runtime-v1"


class TacticalAssessmentRuntimeError(ValueError):
    pass


def seed_tactical_assessment_runtime(conn: sqlite3.Connection) -> None:
    """Seed the second low-risk represented Skill gameplay target and action.

    The simulator is deliberately distinct from Tactical training/practice targets.
    It represents a bounded information packet and decision objective; completing it
    is application evidence only and never becomes Tactical learning evidence here.
    """
    ...
    if conn.execute(
        ...
        raise TacticalAssessmentRuntimeError(
            f"Tactical assessment simulator room {SIMULATOR_ROOM_ID!r} does not exist"
        )
    ...
    conn.execute(
        ...
        (
            SIMULATOR_ID,
            "object",
            "Tactical Situation Assessment Simulator",
...
def _target_capabilities(conn: sqlite3.Connection, target_id: str) -> tuple[str, ...]:
    ...
    if row is None:
        raise TacticalAssessmentRuntimeError(f"Tactical assessment target {target_id!r} does not exist")
    try:
        values = json.loads(row["capabilities_json"] or "[]")
    except (TypeError, json.JSONDecodeError) as exc:
        raise TacticalAssessmentRuntimeError(
            f"Tactical assessment target {target_id!r} has invalid capabilities JSON"
        ) from exc
    if not isinstance(values, list) or not all(isinstance(item, str) and item for item in values):
        raise TacticalAssessmentRuntimeError(
            f"Tactical assessment target {target_id!r} has malformed capabilities"
        )
    return tuple(sorted(set(values)))


def assess_tactical_assessment_action(
...
def validate_tactical_assessment_action(
    conn: sqlite3.Connection,
    actor_id: str,
    target_id: str | None,
) -> RepresentedSkillTaskInstanceAssessment:
    if not target_id:
        raise TacticalAssessmentRuntimeError("Assess action requires a target")
    assessment = assess_tactical_assessment_action(conn, actor_id, target_id)
    if assessment.status == "unsupported":
        raise TacticalAssessmentRuntimeError(
            "Actor does not meet the represented tactical assessment capability contract: "
            + ", ".join(assessment.reasons)
        )
    return assessment


def tactical_assessment_outcome(
    conn: sqlite3.Connection,
    actor_id: str,
    target_id: str,
) -> dict[str, Any]:
    """Resolve one deterministic represented tactical-assessment outcome."""

    task = validate_tactical_assessment_action(conn, actor_id, target_id)
    capability = task.capability
    performance = assess_actor_cognitive_performance(
        conn,
        actor_id,
        "tactical_planning",
        "assess_tactical_situation",
    )
...
def tactical_assessment_application_evidence(
    ...
    *,
    ...
    return {
        ...
        "application_id": "assess_tactical_situation",
