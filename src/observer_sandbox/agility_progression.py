...
AGILITY_FIELD_KEY = "raps_pa.agility"
AGILITY_METHOD_ID = "speed_agility_drills"
SETTLEMENT_EVENT_TYPE = "agility_progression_settled"
SETTLEMENT_SOURCE = "agility-progression-settlement-v1"
STIMULUS_SOURCE = "agility-stimulus-speed-agility-drills-v1"

STIMULUS_MINUTES_PER_UNIT = 30.0
NATURAL_CEILING = 100.0
LEVEL_EXPONENT = 1.6
BASE_POSITIVE_SCALE = 0.16
SATURATION_WINDOW_HOURS = 72.0
SATURATION_COEFFICIENT = 0.25
FULL_RECOVERY_HOURS = 20.0
DETRAINING_GRACE_DAYS = 12.0
DETRAINING_TIME_CONSTANT_DAYS = 50.0
DETRAINING_LEVEL_EXPONENT = 1.4
BASE_DETRAINING_POINTS_PER_DAY = 0.012
MAX_RAW_AGILITY = 100.0


@dataclass(frozen=True)
class AgilityStimulusEvent:
    event_id: int
    sim_time: str
...
def agility_level_factor(current_value: float | int) -> float:
...
def agility_saturation_factor(recent_units: float | int) -> float:
...
def _stimulus_from_payload(payload: dict[str, Any]) -> tuple[float, float] | None:
    ...
    if not isinstance(method, dict) or method.get("method_id") != AGILITY_METHOD_ID:
        return None
    if method.get("source") != "training-method-semantics-v1":
        return None
    if method.get("workload_channels") != ["conditioning", "movement"]:
        return None
    tags = method.get("tags")
    if not isinstance(tags, list) or "speed_agility" not in tags:
        return None
...
def agility_stimulus_events(
    ...
    *,
    ...
    for row in rows:
        ...
        if units > 0.0:
            found.append(AgilityStimulusEvent(int(row["id"]), row["sim_time"], units, minutes))
    found.sort(key=lambda item: (_dt(item.sim_time), item.event_id))
    return found


def recent_agility_stimulus_units(
    conn: sqlite3.Connection,
    actor_id: str,
    *,
    as_of_sim_time: str,
    ...
    total = 0.0
    for event in agility_stimulus_events(conn, actor_id, as_of_sim_time=as_of_sim_time):
...
def latest_agility_settlement_time(conn: sqlite3.Connection, actor_id: str) -> str | None:
...
def _read_agility(conn: sqlite3.Connection, actor_id: str) -> float:
    row = conn.execute(
        "SELECT value_json FROM character_profile_values WHERE entity_id=? AND field_key=?",
        (actor_id, AGILITY_FIELD_KEY),
    ).fetchone()
    if row is None:
        raise KeyError(f"Missing Agility profile value for {actor_id}")
...
def _write_agility(
    ...
    *,
    ...
    conn.execute(
        ...
        (value_json, SETTLEMENT_SOURCE, SETTLEMENT_SOURCE, sim_time, actor_id, AGILITY_FIELD_KEY),
    )
    conn.execute(
...
def integrated_agility_detraining_days(
    conn: sqlite3.Connection,
    actor_id: str,
    *,
    start_sim_time: str,
    ...
    if end <= start:
        return 0.0
    events = agility_stimulus_events(conn, actor_id, as_of_sim_time=end_sim_time)
...
def settle_agility_progression(
    conn: sqlite3.Connection,
    actor_id: str,
    *,
    as_of_sim_time: str,
    ...
    as_of = _dt(as_of_sim_time)
    old_agility = _read_agility(conn, actor_id)
    ...
    if last_settled is None:
        ...
        event_id = record_event(
            ...
            payload={
                ...
                "old_agility": round(old_agility, 6),
                "new_agility": round(old_agility, 6),
                "positive_delta": 0.0,
                "negative_delta": 0.0,
                "net_delta": 0.0,
            },
    ...
    if as_of < last_dt:
        raise ValueError("cannot settle Agility progression before the latest settlement cursor")
    if as_of == last_dt:
        return {
            "status": "no_change",
            "old_agility": round(old_agility, 6),
    ...
    for event in stimuli:
        ...
        if age_hours < FULL_RECOVERY_HOURS or fatigue_blocked or state_quality <= 0.0:
            continue
        level = agility_level_factor(current)
        recent_units = recent_agility_stimulus_units(conn, actor_id, as_of_sim_time=event.sim_time)
        saturation = agility_saturation_factor(recent_units)
        gain = max(0.0, BASE_POSITIVE_SCALE * event.stimulus_units * level * saturation * state_quality)
        current = min(MAX_RAW_AGILITY, current + gain)
        positive_delta += gain
    ...
    integrated_days = integrated_agility_detraining_days(
        conn,
        actor_id,
    ...
    net_delta = current - old_agility

    if abs(net_delta) >= 0.0000005:
        _write_agility(
            conn,
            actor_id,
            old_value=old_agility,
            new_value=current,
            sim_time=as_of_sim_time,
            reason="Agility progression settlement from authored speed/agility drill evidence",
        )
