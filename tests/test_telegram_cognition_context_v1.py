...
def test_character_menu_places_owner_context_button_under_profile_and_keeps_allowed_users_out(tmp_path, monkeypatch):
    db = tmp_path / "observer.sqlite3"
    initialize(db)
    ...
    with connect(db) as conn:
        ...
        assert owner_keyboard[2][0]["callback_data"] == f"ctl:restore_prompt:{ACTOR}"
