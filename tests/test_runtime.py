...
def test_legacy_spatial_ids_are_cleanly_reset_and_runtime_is_remapped(tmp_path):
    ...
    with connect(db) as conn:
        ...
        conn.execute(
            "INSERT INTO entities(id, entity_type, name, capabilities_json) VALUES('room_kitchen','location','Legacy Kitchen','[]')"
        )
        set_field(conn, "char_darian", "runtime.location", "room_kitchen")
        set_field(conn, "char_darian", "runtime.current_action", "move")
        conn.execute(
            "INSERT OR REPLACE INTO runtime_state(key, value_json) VALUES('paused', 'false')"
        )
        conn.execute(
    ...
    # First initialize performs the destructive identity migration and leaves the
    # runtime paused so an old service cannot race stale ids during deployment.
    initialize(db)
    with connect(db) as conn:
        assert json.loads(conn.execute("SELECT value_json FROM runtime_state WHERE key='paused'").fetchone()[0]) is True
        assert json.loads(conn.execute("SELECT value_json FROM runtime_state WHERE key='autonomy_pending_action'").fetchone()[0]) is None
        assert conn.execute("SELECT 1 FROM entities WHERE id='room_kitchen'").fetchone() is None
        location = json.loads(conn.execute(
            "SELECT value_json FROM fields WHERE entity_id='char_darian' AND field_key='runtime.location'"
        ).fetchone()[0])
        assert location == "loc_thorne_estate_kitchen"

    # The next initialize represents the restarted new service and restores the
    # user's previous pause state after the scoped-id graph is safely live.
    initialize(db)
    result = status(db)
    assert result.runtime_state["paused"] is False
    assert result.runtime_state["world_id"] == "world_observer_universe"
