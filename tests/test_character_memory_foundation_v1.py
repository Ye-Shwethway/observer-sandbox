...
def _insert_completed_event(conn, *, sim_time: str, location_id: str, action: str = "rest") -> int:
    cur = conn.execute(
        ...
        (
            ...
            "char_darian",
            "action_completed",
            json.dumps({"action": action,"duration_minutes": 30,"reason": "represented completion fixture"}),
            location_id,
...
def test_schema_encodes_completed_action_memory_once_and_is_idempotent(tmp_path):
    db = tmp_path / "observer.sqlite3"
    initialize(db)
    with connect(db) as conn:
        state = snapshot(conn, "char_darian")
        event_id = _insert_completed_event(conn,sim_time=str(state["sim_time"]),location_id=str(state["location"]))
        conn.commit()
        row = conn.execute(
            """SELECT memory_id,character_id,memory_type,source_event_id,event_sim_time,recall_count
               FROM character_memories WHERE source_event_id=?""",(event_id,),
        ).fetchone()
        ...
        migrate(conn)
        assert conn.execute("SELECT COUNT(*) FROM character_memories WHERE source_event_id=?",(event_id,)).fetchone()[0] == 1
...
def test_retrieval_is_bounded_actor_scoped_and_updates_only_selected_recall_metadata(tmp_path):
    ...
    with connect(db) as conn:
        ...
        first = _insert_completed_event(conn,sim_time="2025-05-10T08:00:00",location_id=location_id,action="rest")
        second = _insert_completed_event(conn,sim_time="2025-05-10T09:00:00",location_id=location_id,action="read")
        conn.commit()
        memories = retrieve_relevant_memories(conn,"char_darian",current_sim_time="2025-05-10T09:30:00",current_location_id=location_id,available_actions=["read", "rest"],limit=1,record_recall=True)
        assert len(memories) == 1
        selected_id = memories[0]["memory_id"]
        rows = conn.execute("SELECT memory_id,source_event_id,recall_count,last_recalled_sim_time FROM character_memories WHERE source_event_id IN (?,?)",(first, second)).fetchall()
...
def test_memory_aware_provider_injects_memory_without_recalling_in_non_capture_mode(tmp_path, monkeypatch):
    ...
    with connect(db) as conn:
        state = snapshot(conn, "char_darian")
        _insert_completed_event(conn,sim_time=str(state["sim_time"]),location_id=str(state["location"]),action="rest")
        conn.commit()
        monkeypatch.setattr(ModelDecisionProvider,"_enrich_state",lambda self, raw_state: {"action_options": [{"action": "rest", "target": None}]})
        provider = MemoryAwareDecisionProvider(conn,character_id="char_darian",capture_context=False)
...
def test_telegram_character_menu_exposes_live_memory_view(tmp_path):
    ...
    with connect(db) as conn:
        state = snapshot(conn, "char_darian")
        _insert_completed_event(conn,sim_time=str(state["sim_time"]),location_id=str(state["location"]),action="observe")
        conn.commit()
        keyboard = character_keyboard("char_darian")
        assert any(button["text"] == "🗃️ Memory" for row in keyboard for button in row)
