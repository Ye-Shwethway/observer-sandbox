...
def test_mind_wakes_only_at_decision_boundaries(tmp_path):
    db = tmp_path / "observer.sqlite3"; initialize(db); provider = FixedProvider()
    with connect(db) as conn:
        set_autonomy_enabled(conn, True)
        planned = autonomy_tick(conn, provider=provider, now_wall=1000)
        assert planned["state"] == "planned" and provider.calls == 1
        assert actor_runtime(conn, "char_darian")["cognition_stats"]["decision_calls"] == 1
        ...
        assert status["state"] == "planned" and provider.calls == 2
        assert actor_runtime(conn, "char_darian")["cognition_stats"]["decision_calls"] == 2
